...
if [[ ! -e scripts/eos ]]; then
  ...
  TOOLING_BACKUP=".eos/history/tooling/scripts-eos-pre-v0.5-$(date -u +%Y%m%dT%H%M%SZ).sh"
...
if [[ -f tools/eos/eos.py ]]; then
  EOS_PY_BACKUP=".eos/history/tooling/eos-py-pre-v0.5-$(date -u +%Y%m%dT%H%M%SZ).py"
