...
write_artifact "engineering/lifecycle/EOSV-verification-v2.md" "EOSV-V2" "EOSV Verification v2" "lifecycle-extension" "governance-authoritative" <<'EOF'
...
{
  "eos_tool_version": "0.7.0",
  "eos_schema_version": "1.1.0",
...
write_managed_file ".eos/domain-model.json" <<'EOF'
{
  ...
  "entities": {
    ...
    "EVID": {
      ...
      "registry": ".eos/evidence.tsv",
      "schema": ".eos/schemas/evid.schema.json",
      "state_machine": ".eos/state-machines/evid.json",
      "parent": null
    }
  },
  "authority": {
...
write_managed_file ".eos/schemas/evid.schema.json" <<'EOF'
{
  "$schema": "https://json-schema.org/draft/2020-12/schema",
  "$id": "eos://schemas/evid/1.0.0",
  "type": "object",
  "required": ["id", "path", "target", "validator", "profile", "kind", "status", "result", "source_hash", "environment_hash", "artifact_hash", "created", "updated"],
  "properties": {
...
write_managed_file ".eos/state-machines/evid.json" <<'EOF'
{
  "kind": "EVID",
  "version": "1.0.0",
  "initial_state": "CAPTURED",
  "terminal_states": ["FAILED", "SUPERSEDED"],
  "states": ["CAPTURED", "VALIDATED", "FAILED", "STALE", "SUPERSEDED"],
  "transitions": {
    "CAPTURED": ["VALIDATED", "FAILED"],
    "VALIDATED": ["STALE", "SUPERSEDED"],
    "FAILED": ["SUPERSEDED"],
    "STALE": ["SUPERSEDED"],
    "SUPERSEDED": []
  }
...
[[ -e .eos/evidence.tsv ]] || printf 'id\tpath\ttarget\texecution\tvalidator\tprofile\tkind\tstatus\tresult\texit_code\tcommand\tsource_hash\tenvironment_hash\tartifact_hash\tcovers\tcreated\tupdated\n' > .eos/evidence.tsv
[[ -e .eos/evidence-links.tsv ]] || printf 'evidence_id\treference\trelation\tcreated\tactor\n' > .eos/evidence-links.tsv
[[ -e .eos/performance-baselines.tsv ]] || printf 'id\tname\ttarget\tunit\tdirection\tbaseline\ttolerance\tcreated\tupdated\n' > .eos/performance-baselines.tsv
...
write_managed_file ".eos/validators.json" <<'EOF'
{
  "version": "2.0.0",
  "validators": {
    "eos-integrity": {"kind": "integrity", "runner": "builtin", "builtin": "core-verify", "description": "Strict EOS structural and lifecycle integrity."},
    "repository": {"kind": "repository", "runner": "legacy-lines", "config": ".eos/validation.commands", "description": "Repository-specific build/test/lint/typecheck commands."},
    "execution-acceptance": {"kind": "acceptance", "runner": "builtin", "builtin": "execution-acceptance", "description": "Structured acceptance-criterion results from the active EXEC session."},
    "secret-scan": {"kind": "security", "runner": "builtin", "builtin": "secret-scan", "description": "High-confidence repository secret-pattern scan without exposing secret values."},
    "supply-chain": {"kind": "supply-chain", "runner": "builtin", "builtin": "supply-chain", "description": "Manifest/lockfile inventory and cryptographic fingerprint evidence."},
    "reproducibility": {"kind": "reproducibility", "runner": "builtin", "builtin": "reproducibility", "description": "Repeat configured deterministic commands and compare normalized outputs."},
    "performance": {"kind": "performance", "runner": "builtin", "builtin": "performance", "description": "Run configured numeric benchmarks against governed performance baselines."}
  }
}
EOF

write_managed_file ".eos/validation-profiles.json" <<'EOF'
{
  "version": "2.0.0",
  "profiles": {
    "default": {"required": ["eos-integrity"], "optional": ["repository"]},
    "wp": {"required": ["eos-integrity"], "optional": ["repository", "execution-acceptance"]},
    "security": {"required": ["eos-integrity", "secret-scan", "supply-chain"], "optional": []},
    "release": {"required": ["eos-integrity", "repository", "secret-scan", "supply-chain"], "optional": ["execution-acceptance"]},
    "reproducibility": {"required": ["eos-integrity", "reproducibility"], "optional": []},
    "performance": {"required": ["eos-integrity", "performance"], "optional": []}
  }
}
EOF

if [[ ! -e .eos/validators.local.json ]]; then
  cat > .eos/validators.local.json <<'EOF'
{
  "validators": {}
}
EOF
...
if [[ ! -e .eos/reproducibility.commands ]]; then
  cat > .eos/reproducibility.commands <<'EOF'
# Commands expected to produce deterministic exit status and normalized output.
# Each command is executed twice when the reproducibility profile is selected.
# Example:
# ./scripts/build-schema.sh --check
EOF
fi

if [[ ! -e .eos/performance.commands ]]; then
  cat > .eos/performance.commands <<'EOF'
# Tab-separated: name<TAB>unit<TAB>command
# Command stdout must contain exactly one numeric measurement.
# Baselines are governed through `./scripts/eos performance record`.
# Example:
# graph_lookup_ms\tms\t./scripts/benchmark-graph.sh
EOF
fi
...
append_artifact_registry EOSV-V2 engineering/lifecycle/EOSV-verification-v2.md lifecycle-extension governance-authoritative
...
if [[ ! -e scripts/eos ]]; then
  ...
  TOOLING_BACKUP=".eos/history/tooling/scripts-eos-pre-v0.7-$(date -u +%Y%m%dT%H%M%SZ).sh"
...
case "${1:-}" in
  ...
  validate|validators|validation|evidence|performance|security|supply-chain|verify)
    exec python3 "$ROOT/tools/eos/verification_v2.py" "$@"
    ;;
  *)
...
if [[ -f tools/eos/eos.py ]]; then
  EOS_PY_BACKUP=".eos/history/tooling/eos-py-pre-v0.7-$(date -u +%Y%m%dT%H%M%SZ).py"
...
if [[ -f tools/eos/execution_v2.py ]]; then
  EOSE_PY_BACKUP=".eos/history/tooling/execution-v2-pre-v0.7-$(date -u +%Y%m%dT%H%M%SZ).py"
...
if [[ -f tools/eos/verification_v2.py ]]; then
  EOSV_PY_BACKUP=".eos/history/tooling/verification-v2-pre-v0.7-$(date -u +%Y%m%dT%H%M%SZ).py"
  mkdir -p "$(dirname "$EOSV_PY_BACKUP")"
  cp tools/eos/verification_v2.py "$EOSV_PY_BACKUP"
fi

cat > tools/eos/verification_v2.py <<'EOSV_PY'
#!/usr/bin/env python3
from __future__ import annotations
...
EOSV_PY
chmod +x tools/eos/verification_v2.py
