...
In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Recovery** dependable. The primary quality concerns are readiness, recovery, runbooks, service objectives, support.
...
- Recovery has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/operational-documentation/recovery.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
