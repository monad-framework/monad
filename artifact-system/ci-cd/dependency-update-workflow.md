...
This artifact defines inventory, ownership, version, risk, update, compatibility, and exit requirements. It exists within Monad's ci/cd concern, which covers repeatable automated validation, integration, packaging, release, provenance, and deployment gates that reproduce local engineering evidence in controlled environments. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Dependency Update Workflow** dependable. The primary quality concerns are local parity, quality gates, provenance, least privilege, release safety.
...
- Dependency Update Workflow has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/ci-cd/dependency-update-workflow.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
