...
This artifact defines the evidence, artifacts, compatibility, provenance, approval, publication, and rollback needed for release. It exists within Monad's ci/cd concern, which covers repeatable automated validation, integration, packaging, release, provenance, and deployment gates that reproduce local engineering evidence in controlled environments. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Release Pipeline** dependable. The primary quality concerns are local parity, quality gates, provenance, least privilege, release safety.
...
- Release Pipeline has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/ci-cd/release-pipeline.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
