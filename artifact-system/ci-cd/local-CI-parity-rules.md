...
In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Local CI Parity Rules** dependable. The primary quality concerns are local parity, quality gates, provenance, least privilege, release safety.
...
- Local CI Parity Rules has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/ci-cd/local-CI-parity-rules.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
