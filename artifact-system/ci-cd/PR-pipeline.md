...
In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **PR Pipeline** dependable. The primary quality concerns are local parity, quality gates, provenance, least privilege, release safety.
...
- PR Pipeline has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/ci-cd/PR-pipeline.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
