...
**Status:** Draft  
**Artifact class:** CI/CD / Engineering Artifact  
...
This artifact defines the core contract; it also defines branch purpose, naming, lifetime, protection, synchronization, and merge expectations; and defines model-agnostic AI integration boundaries, safety, data handling, provenance, evaluation, cost, and human authority. It exists within Monad's ci/cd concern, which covers repeatable automated validation, integration, packaging, release, provenance, and deployment gates that reproduce local engineering evidence in controlled environments. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Main Branch Pipeline** dependable. The primary quality concerns are local parity, quality gates, provenance, least privilege, release safety.
...
- Main Branch Pipeline has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/ci-cd/main-branch-pipeline.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
