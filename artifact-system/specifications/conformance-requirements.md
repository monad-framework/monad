...
In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Conformance Requirements** dependable. The primary quality concerns are normative language, lifecycle, traceability, conformance, compatibility.
...
- Conformance Requirements has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/specifications/conformance-requirements.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
