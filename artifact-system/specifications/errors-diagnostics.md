...
**Status:** Draft  
**Artifact class:** Specification System / Engineering Artifact  
...
This artifact defines the core contract; it also defines structured failure or warning information, severity, provenance, remediation, and machine representation; and defines error classes, stable codes, safe disclosure, retry semantics, and remediation. It exists within Monad's specification system concern, which covers authoring, lifecycle, normative terminology, dependencies, traceability, conformance, schemas, examples, compatibility, and publication of specifications. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Errors Diagnostics** dependable. The primary quality concerns are normative language, lifecycle, traceability, conformance, compatibility.
...
- Errors Diagnostics has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/specifications/errors-diagnostics.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
