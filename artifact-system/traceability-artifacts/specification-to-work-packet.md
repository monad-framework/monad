...
**Status:** Draft  
**Artifact class:** Traceability / Specification  
...
In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Specification To Work Packet** dependable. The primary quality concerns are stable identifiers, bidirectional links, coverage, provenance, impact.
...
- Specification To Work Packet has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/traceability-artifacts/specification-to-work-packet.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
