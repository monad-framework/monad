...
In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Code To Tests** dependable. The primary quality concerns are stable identifiers, bidirectional links, coverage, provenance, impact.
...
- Code To Tests has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/traceability-artifacts/code-to-tests.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
