...
This artifact defines branch purpose, naming, lifetime, protection, synchronization, and merge expectations. It exists within Monad's traceability concern, which covers end-to-end relationships from intent and requirements through decisions, specifications, work, implementation, tests, releases, diagnostics, and observed evidence. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Work Packet To Branch** dependable. The primary quality concerns are stable identifiers, bidirectional links, coverage, provenance, impact.
...
- Work Packet To Branch has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/traceability-artifacts/work-packet-to-branch.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
