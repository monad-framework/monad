...
This artifact defines observable conditions and evidence required before a result may be accepted. It exists within Monad's milestones concern, which covers the milestones concern within Monad's engineering knowledge compilation and operating model. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Milestone Acceptance Criteria** dependable. The primary quality concerns are explicit scope, stable identity, traceability, review, verification.
...
- Milestone Acceptance Criteria has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/milestones/milestone-acceptance-criteria.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
