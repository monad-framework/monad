...
**Status:** Draft  
**Artifact class:** Milestones / Engineering Artifact  
...
This artifact defines the core contract; it also defines allowed relationship types, direction, cardinality, invariants, and provenance; and defines the evidence, artifacts, compatibility, provenance, approval, publication, and rollback needed for release. It exists within Monad's milestones concern, which covers the milestones concern within Monad's engineering knowledge compilation and operating model. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Milestone Release Relationship** dependable. The primary quality concerns are explicit scope, stable identity, traceability, review, verification.
...
- Milestone Release Relationship has an explicit scope and must not silently absorb neighboring responsibilities.
...
The milestone release relationship model defines named concepts, their stable identities, ownership, lifecycle, and invariants.
...
`artifact-system/milestones/milestone-release-relationship.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
