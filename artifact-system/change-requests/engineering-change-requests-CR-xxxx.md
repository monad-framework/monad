...
**Status:** Draft  
**Artifact class:** Change Request / Engineering Artifact  
...
This artifact defines the stable responsibilities, rules, evidence, and change boundaries for this artifact. It exists within Monad's change request concern, which covers controlled introduction of changes that arise outside an already-authorized work packet or that materially alter accepted scope, authority, risk, or compatibility. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Engineering Change Requests Cr Xxxx** dependable. The primary quality concerns are origin, impact, disposition, authorization, verification.
...
- Engineering Change Requests Cr Xxxx has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/change-requests/engineering-change-requests-CR-xxxx.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
