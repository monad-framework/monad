...
**Status:** Draft  
**Artifact class:** Change Request / Architecture  
...
This artifact defines structural responsibilities, boundaries, flows, interfaces, deployment concerns, and trade-offs. It exists within Monad's change request concern, which covers controlled introduction of changes that arise outside an already-authorized work packet or that materially alter accepted scope, authority, risk, or compatibility. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Affected Specification Architecture** dependable. The primary quality concerns are origin, impact, disposition, authorization, verification.
...
- Affected Specification Architecture has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/change-requests/affected-specification-architecture.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
