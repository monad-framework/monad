...
In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Disposition** dependable. The primary quality concerns are origin, impact, disposition, authorization, verification.
...
- Disposition has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/change-requests/disposition.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
