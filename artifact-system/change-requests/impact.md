...
In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Impact** dependable. The primary quality concerns are origin, impact, disposition, authorization, verification.
...
`artifact-system/change-requests/impact.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
