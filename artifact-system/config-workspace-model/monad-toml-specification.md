...
**Status:** Draft  
**Artifact class:** Configuration and Workspace / Specification  
...
This artifact defines the stable responsibilities, rules, evidence, and change boundaries for this artifact. It exists within Monad's configuration and workspace concern, which covers the canonical model for discovering repositories, resolving workspace state, configuration precedence, profiles, lock state, environment boundaries, and local generated state. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Monad Toml Specification** dependable. The primary quality concerns are discovery, precedence, validation, determinism, migration.
...
- Monad Toml Specification has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/config-workspace-model/monad-toml-specification.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
