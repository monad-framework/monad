...
In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Package Project Discovery** dependable. The primary quality concerns are discovery, precedence, validation, determinism, migration.
...
- Package Project Discovery has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/config-workspace-model/package-project-discovery.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
