...
This artifact defines repository discovery, ownership, package/component boundaries, identity, configuration, and local state. It exists within Monad's configuration and workspace concern, which covers the canonical model for discovering repositories, resolving workspace state, configuration precedence, profiles, lock state, environment boundaries, and local generated state. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Workspace Discovery Rules** dependable. The primary quality concerns are discovery, precedence, validation, determinism, migration.
...
- Workspace Discovery Rules has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/config-workspace-model/workspace-discovery-rules.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
