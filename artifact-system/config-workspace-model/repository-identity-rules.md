...
**Status:** Draft  
**Artifact class:** Configuration and Workspace / Rules  
...
This artifact defines the core contract; it also defines stable identity, scope, uniqueness, persistence, aliases, and change semantics; and defines entity classes, identity, ownership, lifecycle, attributes, and invariants. It exists within Monad's configuration and workspace concern, which covers the canonical model for discovering repositories, resolving workspace state, configuration precedence, profiles, lock state, environment boundaries, and local generated state. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Repository Identity Rules** dependable. The primary quality concerns are discovery, precedence, validation, determinism, migration.
...
- Repository Identity Rules has an explicit scope and must not silently absorb neighboring responsibilities.
...
The repository identity rules model defines named concepts, their stable identities, ownership, lifecycle, and invariants.
...
`artifact-system/config-workspace-model/repository-identity-rules.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
