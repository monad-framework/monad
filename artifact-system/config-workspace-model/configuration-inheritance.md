...
In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Configuration Inheritance** dependable. The primary quality concerns are discovery, precedence, validation, determinism, migration.
...
- Configuration Inheritance has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/config-workspace-model/configuration-inheritance.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
