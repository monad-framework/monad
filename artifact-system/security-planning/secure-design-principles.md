...
**Status:** Draft  
**Artifact class:** Security / Principles  
...
This artifact states durable rules used to evaluate choices and resolve ambiguity. It exists within Monad's security concern, which covers threats, trust boundaries, identities, permissions, secrets, supply-chain controls, vulnerability management, sandboxing, and security evidence. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Secure Design Principles** dependable. The primary quality concerns are least privilege, trust, secrets, supply chain, incident response.
...
- Secure Design Principles has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/security-planning/secure-design-principles.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
