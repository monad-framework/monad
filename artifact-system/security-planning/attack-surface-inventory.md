...
This artifact defines the stable responsibilities, rules, evidence, and change boundaries for this artifact. It exists within Monad's security concern, which covers threats, trust boundaries, identities, permissions, secrets, supply-chain controls, vulnerability management, sandboxing, and security evidence. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Attack Surface Inventory** dependable. The primary quality concerns are least privilege, trust, secrets, supply chain, incident response.
...
- Attack Surface Inventory has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/security-planning/attack-surface-inventory.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
