...
This artifact defines a sequenced, owned path from current state to an explicit outcome and decision gate. It exists within Monad's security concern, which covers threats, trust boundaries, identities, permissions, secrets, supply-chain controls, vulnerability management, sandboxing, and security evidence. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Artifact Signing Plan** dependable. The primary quality concerns are least privilege, trust, secrets, supply chain, incident response.
...
- Artifact Signing Plan has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/security-planning/artifact-signing-plan.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
