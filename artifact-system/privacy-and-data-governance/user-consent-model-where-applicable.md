...
**Status:** Draft  
**Artifact class:** Repository and Artifact Governance / Model  
...
This artifact defines entities, relationships, states, invariants, ownership, and allowed transitions. It exists within Monad's repository and artifact governance concern, which covers repository structure, artifact authority, naming, versioning, branching, generated-content, contribution, and change-control conventions. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **User Consent Model Where Applicable** dependable. The primary quality concerns are authority, naming, lifecycle, traceability, review.
...
- User Consent Model Where Applicable has an explicit scope and must not silently absorb neighboring responsibilities.
...
The user consent model where applicable model defines named concepts, their stable identities, ownership, lifecycle, and invariants.
...
`artifact-system/privacy-and-data-governance/user-consent-model-where-applicable.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
