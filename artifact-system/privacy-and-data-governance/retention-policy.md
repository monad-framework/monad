...
In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Retention Policy** dependable. The primary quality concerns are authority, naming, lifecycle, traceability, review.
...
- Retention Policy has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/privacy-and-data-governance/retention-policy.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
