...
In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Data Classification** dependable. The primary quality concerns are authority, naming, lifecycle, traceability, review.
...
- Data Classification has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/privacy-and-data-governance/data-classification.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
