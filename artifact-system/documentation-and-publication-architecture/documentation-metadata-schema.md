...
**Status:** Draft  
**Artifact class:** Documentation and Publication / Schema Contract  
...
This artifact defines the core contract; it also defines machine-verifiable structure, required fields, constraints, and extensibility rules; and defines ownership, classification, lifecycle, quality, lineage, retention, access, and migration expectations. It exists within Monad's documentation and publication concern, which covers the architecture that turns canonical engineering knowledge into trustworthy human-facing documentation, references, generated views, indexes, and searchable publications. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Documentation Metadata Schema** dependable. The primary quality concerns are source of truth, projection, versioning, cross-reference, quality.
...
- Documentation Metadata Schema has an explicit scope and must not silently absorb neighboring responsibilities.
...
The documentation metadata schema model defines named concepts, their stable identities, ownership, lifecycle, and invariants.
...
`artifact-system/documentation-and-publication-architecture/documentation-metadata-schema.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
