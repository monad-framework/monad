...
In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Search Strategy** dependable. The primary quality concerns are source of truth, projection, versioning, cross-reference, quality.
...
- Search Strategy has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/documentation-and-publication-architecture/search-strategy.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
