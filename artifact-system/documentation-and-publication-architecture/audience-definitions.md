...
In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Audience Definitions** dependable. The primary quality concerns are source of truth, projection, versioning, cross-reference, quality.
...
- Audience Definitions has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/documentation-and-publication-architecture/audience-definitions.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
