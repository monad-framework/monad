...
**Status:** Draft  
**Artifact class:** Documentation and Publication / Taxonomy  
...
This artifact defines controlled categories, classification rules, overlap handling, and extension governance. It exists within Monad's documentation and publication concern, which covers the architecture that turns canonical engineering knowledge into trustworthy human-facing documentation, references, generated views, indexes, and searchable publications. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Documentation Taxonomy** dependable. The primary quality concerns are source of truth, projection, versioning, cross-reference, quality.
...
- Documentation Taxonomy has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/documentation-and-publication-architecture/documentation-taxonomy.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
