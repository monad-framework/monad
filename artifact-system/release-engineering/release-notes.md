...
In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Release Notes** dependable. The primary quality concerns are versioning, provenance, signing, compatibility, rollback.
...
- Release Notes has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/release-engineering/release-notes.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
