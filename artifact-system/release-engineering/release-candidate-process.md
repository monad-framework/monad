...
In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Release Candidate Process** dependable. The primary quality concerns are versioning, provenance, signing, compatibility, rollback.
...
- Release Candidate Process has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/release-engineering/release-candidate-process.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
