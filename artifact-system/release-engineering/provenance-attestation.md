...
**Catalog path:** `artifact-system/release-engineering/provenance-attestation.md`  
**Status:** Draft  
...
The **Provenance Attestation** artifact makes provenance attestation explicit and reviewable within Monad. It exists to prevent important release engineering decisions from living only in chat, memory, implementation detail, or tool-specific state.

Within the artifact catalog this document defines the purpose, required information, authority, lifecycle, relationships, validation, and safe use of this engineering artifact. Its family focuses on versioning, release candidates, manifests, signing, SBOM/provenance, compatibility, migrations, rollback, and release readiness. The contract is deliberately independent of a particular implementation tool so humans, ChatGPT, Codex, CI, and future Monad automation can reason about the same engineering intent.

## Activation

Create or promote an instantiated Provenance Attestation when an accepted decision, approved specification, active Work Packet, release gate, recurring operational need, or material risk requires a durable representation. Do not activate the artifact merely because the catalog contains this contract.
...
Verification for an instantiated Provenance Attestation includes:
