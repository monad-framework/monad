...
This artifact defines origin, transformation history, evidence linkage, integrity, and reconstruction requirements. It exists within Monad's release engineering concern, which covers versioning, release readiness, packaging, signing, SBOMs, attestations, notes, compatibility, migrations, rollback, and artifact publication. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Provenance Attestation** dependable. The primary quality concerns are versioning, provenance, signing, compatibility, rollback.
...
- Provenance Attestation has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/release-engineering/provenance-attestation.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
