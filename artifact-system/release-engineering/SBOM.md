...
In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **SBOM** dependable. The primary quality concerns are versioning, provenance, signing, compatibility, rollback.
...
- SBOM has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/release-engineering/SBOM.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
