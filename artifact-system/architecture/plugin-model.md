...
This artifact defines the core contract; it also defines entities, relationships, states, invariants, ownership, and allowed transitions; and defines extension discovery, manifest, capability, permissions, lifecycle, compatibility, isolation, and trust. It exists within Monad's architecture concern, which covers the structural model of Monad: its boundaries, components, planes, data flows, responsibilities, deployment shapes, and cross-cutting quality constraints. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Plugin Model** dependable. The primary quality concerns are boundaries, responsibility, quality attributes, trade-offs, evolution.
...
- Plugin Model has an explicit scope and must not silently absorb neighboring responsibilities.
...
The plugin model model defines named concepts, their stable identities, ownership, lifecycle, and invariants.
...
`artifact-system/architecture/plugin-model.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
