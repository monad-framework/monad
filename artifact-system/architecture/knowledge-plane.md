...
In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Knowledge Plane** dependable. The primary quality concerns are boundaries, responsibility, quality attributes, trade-offs, evolution.
...
- Knowledge Plane has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/architecture/knowledge-plane.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
