...
This artifact defines entities, relationships, states, invariants, ownership, and allowed transitions. It exists within Monad's architecture concern, which covers the structural model of Monad: its boundaries, components, planes, data flows, responsibilities, deployment shapes, and cross-cutting quality constraints. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Remote Execution Model** dependable. The primary quality concerns are boundaries, responsibility, quality attributes, trade-offs, evolution.
...
- Remote Execution Model has an explicit scope and must not silently absorb neighboring responsibilities.
...
The remote execution model model defines named concepts, their stable identities, ownership, lifecycle, and invariants.
...
`artifact-system/architecture/remote-execution-model.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
