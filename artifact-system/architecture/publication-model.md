...
In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Publication Model** dependable. The primary quality concerns are boundaries, responsibility, quality attributes, trade-offs, evolution.
...
- Publication Model has an explicit scope and must not silently absorb neighboring responsibilities.
...
The publication model model defines named concepts, their stable identities, ownership, lifecycle, and invariants.
...
`artifact-system/architecture/publication-model.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
