...
This artifact defines entities, relationships, states, invariants, ownership, and allowed transitions. It exists within Monad's ai and agent architecture concern, which covers bounded use of AI systems and software agents inside Monad while preserving human authority, provenance, least privilege, reproducibility, and inspectability. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Model Independent Interfaces** dependable. The primary quality concerns are capabilities, permissions, context, provenance, human approval.
...
- Model Independent Interfaces has an explicit scope and must not silently absorb neighboring responsibilities.
...
The model independent interfaces model defines named concepts, their stable identities, ownership, lifecycle, and invariants.
...
`artifact-system/ai-and-agent-architecture/model-independent-interfaces.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
