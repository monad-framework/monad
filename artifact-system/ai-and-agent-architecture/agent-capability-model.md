...
**Status:** Draft  
**Artifact class:** AI and Agent Architecture / Model  
**Owner:** Architecture Owner  
**Required reviewers:** Engineering Owner, Architecture Owner, Security Owner  
...
This artifact defines the core contract; it also defines entities, relationships, states, invariants, ownership, and allowed transitions; and defines an independently understandable ability, its boundaries, dependencies, inputs, outputs, and maturity; and defines bounded machine actor identity, authority, task scope, context, provenance, escalation, and review. It exists within Monad's ai and agent architecture concern, which covers bounded use of AI systems and software agents inside Monad while preserving human authority, provenance, least privilege, reproducibility, and inspectability. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Agent Capability Model** dependable. The primary quality concerns are capabilities, permissions, context, provenance, human approval.
...
- Agent Capability Model has an explicit scope and must not silently absorb neighboring responsibilities.
...
The agent capability model model defines named concepts, their stable identities, ownership, lifecycle, and invariants.
...
`artifact-system/ai-and-agent-architecture/agent-capability-model.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
