...
**Status:** Draft  
**Artifact class:** AI and Agent Architecture / Review Contract  
...
This artifact defines the core contract; it also defines mandatory or conditionally mandatory rules, enforcement points, exceptions, and evidence; and defines entry criteria, reviewers, evidence, findings, disposition, decision authority, and closure conditions; and defines model-agnostic AI integration boundaries, safety, data handling, provenance, evaluation, cost, and human authority. It exists within Monad's ai and agent architecture concern, which covers bounded use of AI systems and software agents inside Monad while preserving human authority, provenance, least privilege, reproducibility, and inspectability. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **AI Review Policy** dependable. The primary quality concerns are capabilities, permissions, context, provenance, human approval.
...
- AI Review Policy has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/ai-and-agent-architecture/AI-review-policy.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
