...
**Status:** Draft  
**Artifact class:** AI and Agent Architecture / Rules  
...
In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Escalation Rules** dependable. The primary quality concerns are capabilities, permissions, context, provenance, human approval.
...
- Escalation Rules has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/ai-and-agent-architecture/escalation-rules.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
