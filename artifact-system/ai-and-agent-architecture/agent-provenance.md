...
This artifact defines the core contract; it also defines origin, transformation history, evidence linkage, integrity, and reconstruction requirements; and defines bounded machine actor identity, authority, task scope, context, provenance, escalation, and review. It exists within Monad's ai and agent architecture concern, which covers bounded use of AI systems and software agents inside Monad while preserving human authority, provenance, least privilege, reproducibility, and inspectability. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Agent Provenance** dependable. The primary quality concerns are capabilities, permissions, context, provenance, human approval.
...
- Agent Provenance has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/ai-and-agent-architecture/agent-provenance.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
