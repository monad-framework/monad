...
In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Model Provider Abstraction** dependable. The primary quality concerns are capabilities, permissions, context, provenance, human approval.
...
- Model Provider Abstraction has an explicit scope and must not silently absorb neighboring responsibilities.
...
The model provider abstraction model defines named concepts, their stable identities, ownership, lifecycle, and invariants.
...
`artifact-system/ai-and-agent-architecture/model-provider-abstraction.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
