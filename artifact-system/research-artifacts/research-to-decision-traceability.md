...
**Status:** Draft  
**Artifact class:** Research / Engineering Artifact  
...
This artifact defines the stable responsibilities, rules, evidence, and change boundaries for this artifact. It exists within Monad's research concern, which covers questions, experiments, evaluations, benchmarks, trade studies, findings, and the controlled promotion of evidence into decisions and specifications. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Research To Decision Traceability** dependable. The primary quality concerns are question, method, evidence, limitations, decision linkage.
...
- Research To Decision Traceability has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/research-artifacts/research-to-decision-traceability.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
