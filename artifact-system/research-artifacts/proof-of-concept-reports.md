...
In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Proof Of Concept Reports** dependable. The primary quality concerns are question, method, evidence, limitations, decision linkage.
...
- Proof Of Concept Reports has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/research-artifacts/proof-of-concept-reports.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
