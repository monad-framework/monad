...
In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Explainability** dependable. The primary quality concerns are execution plans, fingerprints, native tools, isolation, reproducibility.
...
- Explainability has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/build-and-execution/explainability.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
