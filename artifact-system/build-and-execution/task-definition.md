...
In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Task Definition** dependable. The primary quality concerns are execution plans, fingerprints, native tools, isolation, reproducibility.
...
- Task Definition has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/build-and-execution/task-definition.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
