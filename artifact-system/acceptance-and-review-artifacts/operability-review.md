...
In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Operability Review** dependable. The primary quality concerns are explicit criteria, review authority, evidence links, dissent, follow-up actions.
...
- Operability Review has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/acceptance-and-review-artifacts/operability-review.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
