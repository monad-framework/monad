...
This artifact defines the core contract; it also defines structural responsibilities, boundaries, flows, interfaces, deployment concerns, and trade-offs; and defines entry criteria, reviewers, evidence, findings, disposition, decision authority, and closure conditions. It exists within Monad's acceptance and review concern, which covers formal evidence used to decide readiness, acceptance, closure, operability, security, and learning outcomes. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Architecture Review** dependable. The primary quality concerns are explicit criteria, review authority, evidence links, dissent, follow-up actions.
...
- Architecture Review has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/acceptance-and-review-artifacts/architecture-review.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
