...
**Status:** Draft  
**Artifact class:** CLI Design / Architecture  
...
This artifact defines the core contract; it also defines structural responsibilities, boundaries, flows, interfaces, deployment concerns, and trade-offs; and defines command-line information architecture, commands, flags, output, errors, compatibility, and automation use. It exists within Monad's cli design concern, which covers the command-line interaction model through which users inspect, explain, validate, plan, execute, query, and operate Monad. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **CLI Information Architecture** dependable. The primary quality concerns are command taxonomy, stable output, diagnostics, configuration, accessibility.
...
- CLI Information Architecture has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/cli-design/CLI-information-architecture.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
