...
In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Tui Design** dependable. The primary quality concerns are command taxonomy, stable output, diagnostics, configuration, accessibility.
...
- Tui Design has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/cli-design/TUI-design.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
