...
This artifact defines mandatory or conditionally mandatory rules, enforcement points, exceptions, and evidence. It exists within Monad's cli design concern, which covers the command-line interaction model through which users inspect, explain, validate, plan, execute, query, and operate Monad. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Environment Variable Policy** dependable. The primary quality concerns are command taxonomy, stable output, diagnostics, configuration, accessibility.
...
- Environment Variable Policy has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/cli-design/environment-variable-policy.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
