...
**Status:** Draft  
**Artifact class:** GitHub Project Management Artifacts / Rules  
...
This artifact defines the stable responsibilities, rules, evidence, and change boundaries for this artifact. It exists within Monad's github project management artifacts concern, which covers the github project management artifacts concern within Monad's engineering knowledge compilation and operating model. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Issue ↔ Wp ↔ Wc ↔ PI Linkage Rules** dependable. The primary quality concerns are explicit scope, stable identity, traceability, review, verification.
...
- Issue ↔ Wp ↔ Wc ↔ PI Linkage Rules has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/github-project-management-artifacts/issue-↔-WP-↔-WC-↔-PI-linkage-rules.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
