...
In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **KIR Generation Rules** dependable. The primary quality concerns are syntax, semantics, resolution, diagnostics, lowering.
...
- KIR Generation Rules has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/lang-compiler-design/KIR-generation-rules.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
