...
**Status:** Draft  
**Artifact class:** Language and Compiler / Policy  
...
This artifact defines the core contract; it also defines mandatory or conditionally mandatory rules, enforcement points, exceptions, and evidence; and defines which producers and consumers may interoperate and how incompatible change is detected and migrated. It exists within Monad's language and compiler concern, which covers the authoring, parsing, semantic analysis, normalization, resolution, lowering, diagnostics, and compatibility model for Monad's specification and knowledge compilation pipeline. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Compiler Compatibility Policy** dependable. The primary quality concerns are syntax, semantics, resolution, diagnostics, lowering.
...
- Compiler Compatibility Policy has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/lang-compiler-design/compiler-compatibility-policy.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
