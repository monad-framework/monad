...
In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Ast Definition** dependable. The primary quality concerns are syntax, semantics, resolution, diagnostics, lowering.
...
- Ast Definition has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/lang-compiler-design/AST-definition.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
