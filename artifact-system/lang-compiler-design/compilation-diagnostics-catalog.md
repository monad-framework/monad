...
**Status:** Draft  
**Artifact class:** Language and Compiler / Catalog  
...
This artifact defines the core contract; it also defines a discoverable inventory, stable identifiers, ownership, lifecycle, and metadata expectations; and defines structured failure or warning information, severity, provenance, remediation, and machine representation. It exists within Monad's language and compiler concern, which covers the authoring, parsing, semantic analysis, normalization, resolution, lowering, diagnostics, and compatibility model for Monad's specification and knowledge compilation pipeline. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Compilation Diagnostics Catalog** dependable. The primary quality concerns are syntax, semantics, resolution, diagnostics, lowering.
...
- Compilation Diagnostics Catalog has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/lang-compiler-design/compilation-diagnostics-catalog.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
