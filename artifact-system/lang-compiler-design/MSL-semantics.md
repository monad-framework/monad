...
In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **MSL Semantics** dependable. The primary quality concerns are syntax, semantics, resolution, diagnostics, lowering.
...
- MSL Semantics has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/lang-compiler-design/MSL-semantics.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
