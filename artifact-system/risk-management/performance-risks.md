...
This artifact defines the core contract; it also defines representative workloads, budgets, measurement, baselines, regression rules, and capacity evidence; and defines cause-event-consequence, likelihood, impact, owner, treatment, trigger, contingency, and closure. It exists within Monad's risk management concern, which covers identification, analysis, ownership, treatment, triggers, contingencies, escalation, and closure of uncertainty that may affect Monad objectives. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Performance Risks** dependable. The primary quality concerns are cause, event, consequence, treatment, trigger.
...
- Performance Risks has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/risk-management/performance-risks.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
