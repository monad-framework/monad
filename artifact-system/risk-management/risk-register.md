...
**Catalog path:** `artifact-system/risk-management/risk-register.md`  
**Status:** Draft  
...
The **Risk Register** artifact makes risk register explicit and reviewable within Monad. It exists to prevent important risk management decisions from living only in chat, memory, implementation detail, or tool-specific state.

Within the artifact catalog this document defines a controlled inventory, identifiers, ownership, state, update rules, and integrity checks. Its family focuses on risk identification, scoring, ownership, treatment, acceptance, escalation, residual risk, and evidence. The contract is deliberately independent of a particular implementation tool so humans, ChatGPT, Codex, CI, and future Monad automation can reason about the same engineering intent.
