...
**Catalog path:** `artifact-system/foundational-artifacts/vision/knowledge-model-principles.md`  
**Status:** Draft  
...
The **Knowledge Model Principles** artifact makes knowledge model principles explicit and reviewable within Monad. It exists to prevent important foundational artifacts decisions from living only in chat, memory, implementation detail, or tool-specific state.

Within the artifact catalog this document defines concepts, relationships, ownership, invariants, state, boundaries, and interpretation rules. Its family focuses on Monad's constitutional identity, mission, thesis, principles, language, scope, human authority, and long-term system intent. The contract is deliberately independent of a particular implementation tool so humans, ChatGPT, Codex, CI, and future Monad automation can reason about the same engineering intent.

## Activation

Create or promote an instantiated Knowledge Model Principles when an accepted decision, approved specification, active Work Packet, release gate, recurring operational need, or material risk requires a durable representation. Do not activate the artifact merely because the catalog contains this contract.
...
Verification for an instantiated Knowledge Model Principles includes:
