...
**Status:** Draft  
**Artifact class:** Foundational Artifacts / Engineering Artifact  
...
This artifact defines the core contract; it also defines the invariants required for equivalent inputs and environment declarations to produce equivalent results; and defines the evidence and environment capture required to independently reproduce a result. It exists within Monad's foundational artifacts concern, which covers the foundational artifacts concern within Monad's engineering knowledge compilation and operating model. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Determinism And Reproducibility** dependable. The primary quality concerns are explicit scope, stable identity, traceability, review, verification.
...
- Determinism And Reproducibility has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/foundational-artifacts/vision/determinism-and-reproducibility.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
