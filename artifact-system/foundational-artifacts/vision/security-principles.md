...
This artifact defines the core contract; it also states durable rules used to evaluate choices and resolve ambiguity; and defines threats, controls, ownership, verification, exceptions, and incident linkage. It exists within Monad's foundational artifacts concern, which covers the foundational artifacts concern within Monad's engineering knowledge compilation and operating model. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Security Principles** dependable. The primary quality concerns are explicit scope, stable identity, traceability, review, verification.
...
- Security Principles has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/foundational-artifacts/vision/security-principles.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
