...
This artifact states durable rules used to evaluate choices and resolve ambiguity. It exists within Monad's foundational artifacts concern, which covers the foundational artifacts concern within Monad's engineering knowledge compilation and operating model. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Open Ecosystem Principles** dependable. The primary quality concerns are explicit scope, stable identity, traceability, review, verification.
...
- Open Ecosystem Principles has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/foundational-artifacts/vision/open-ecosystem-principles.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
