...
This artifact defines the core contract; it also defines a sequenced, owned path from current state to an explicit outcome and decision gate; and defines minimal recomputation from observed change while preserving correctness and explainability. It exists within Monad's program management concern, which covers the hierarchy and controls used to translate product and architecture intent into milestones, increments, sprints or work cycles, work packets, risks, dependencies, and status. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Product Increment Plan** dependable. The primary quality concerns are outcomes, sequencing, dependencies, risk, evidence.
...
- Product Increment Plan has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/program-management/Product-Increment-plan.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
