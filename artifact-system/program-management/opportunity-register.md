...
This artifact defines a controlled living record with stable IDs, ownership, state, review triggers, and closure criteria. It exists within Monad's program management concern, which covers the hierarchy and controls used to translate product and architecture intent into milestones, increments, sprints or work cycles, work packets, risks, dependencies, and status. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Opportunity Register** dependable. The primary quality concerns are outcomes, sequencing, dependencies, risk, evidence.
...
- Opportunity Register has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/program-management/opportunity-register.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
