...
In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Governance Cadence** dependable. The primary quality concerns are outcomes, sequencing, dependencies, risk, evidence.
...
- Governance Cadence has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/program-management/governance-cadence.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
