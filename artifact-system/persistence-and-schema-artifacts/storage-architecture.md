...
**Catalog path:** `artifact-system/persistence-and-schema-artifacts/storage-architecture.md`  
**Status:** Draft  
...
The **Storage Architecture** artifact makes storage architecture explicit and reviewable within Monad. It exists to prevent important persistence and schema artifacts decisions from living only in chat, memory, implementation detail, or tool-specific state.

Within the artifact catalog this document defines structure, responsibility boundaries, flows, quality attributes, constraints, and evolution rules. Its family focuses on canonical schemas, storage ownership, migrations, durability, compatibility, recovery, indexes, and data integrity. The contract is deliberately independent of a particular implementation tool so humans, ChatGPT, Codex, CI, and future Monad automation can reason about the same engineering intent.

## Activation

Create or promote an instantiated Storage Architecture when an accepted decision, approved specification, active Work Packet, release gate, recurring operational need, or material risk requires a durable representation. Do not activate the artifact merely because the catalog contains this contract.
...
This contract governs the structure, semantics, authority, lifecycle, traceability, and verification of Storage Architecture records used by Monad. It does not grant decision authority, approve an implementation, or supersede higher-order governance. Tool-specific UI fields are projections unless explicitly designated canonical.
...
Verification for an instantiated Storage Architecture includes:
