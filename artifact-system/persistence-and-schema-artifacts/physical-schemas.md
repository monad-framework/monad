...
**Status:** Draft  
**Artifact class:** Persistence And Schema Artifacts / Schema Contract  
...
This artifact defines machine-verifiable structure, required fields, constraints, and extensibility rules. It exists within Monad's persistence and schema artifacts concern, which covers the persistence and schema artifacts concern within Monad's engineering knowledge compilation and operating model. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Physical Schemas** dependable. The primary quality concerns are explicit scope, stable identity, traceability, review, verification.
...
- Physical Schemas has an explicit scope and must not silently absorb neighboring responsibilities.
...
The physical schemas model defines named concepts, their stable identities, ownership, lifecycle, and invariants.
...
`artifact-system/persistence-and-schema-artifacts/physical-schemas.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
