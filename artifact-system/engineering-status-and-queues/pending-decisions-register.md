...
In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Pending Decisions Register** dependable. The primary quality concerns are explicit scope, stable identity, traceability, review, verification.
...
- Pending Decisions Register has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/engineering-status-and-queues/pending-decisions-register.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
