...
**Status:** Draft  
**Artifact class:** Kernel Intermediate Representation / Schema Contract  
...
This artifact defines machine-verifiable structure, required fields, constraints, and extensibility rules. It exists within Monad's kernel intermediate representation concern, which covers the canonical, deterministic intermediate representation that bridges semantic engineering knowledge and downstream execution, publication, analysis, and interoperability. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Schema** dependable. The primary quality concerns are canonical semantics, stable identity, deterministic serialization, compatibility, validation.
...
- Schema has an explicit scope and must not silently absorb neighboring responsibilities.
...
The schema model defines named concepts, their stable identities, ownership, lifecycle, and invariants.
...
`artifact-system/KIR/schema.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
