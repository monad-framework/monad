...
This artifact defines canonical form so equivalent inputs converge on one deterministic representation. It exists within Monad's kernel intermediate representation concern, which covers the canonical, deterministic intermediate representation that bridges semantic engineering knowledge and downstream execution, publication, analysis, and interoperability. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Canonicalization** dependable. The primary quality concerns are canonical semantics, stable identity, deterministic serialization, compatibility, validation.
...
- Canonicalization has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/KIR/canonicalization.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
