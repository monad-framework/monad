...
**Status:** Draft  
**Artifact class:** Kernel Intermediate Representation / Rules  
...
- Identity Rules has an explicit scope and must not silently absorb neighboring responsibilities.
...
The identity rules model defines named concepts, their stable identities, ownership, lifecycle, and invariants.
...
`artifact-system/KIR/identity-rules.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
