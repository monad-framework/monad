...
**Catalog path:** `artifact-system/developer-experience/error-message-guidelines.md`  
**Status:** Draft  
**Artifact class:** guide  
**Owner:** Engineering Owner  
**Authority:** Describes an artifact contract; it is not automatically an instantiated or accepted project record.

## Purpose

The **Error Message Guidelines** artifact makes error message guidelines explicit and reviewable within Monad. It exists to prevent important developer experience decisions from living only in chat, memory, implementation detail, or tool-specific state.

Within the artifact catalog this document defines executable guidance, prerequisites, steps, expected outcomes, failures, recovery, and escalation. Its family focuses on installation, onboarding, feedback, local workflows, shell/IDE integration, diagnostics, discoverability, and low-friction safe operation. The contract is deliberately independent of a particular implementation tool so humans, ChatGPT, Codex, CI, and future Monad automation can reason about the same engineering intent.

## Activation

Create or promote an instantiated Error Message Guidelines when an accepted decision, approved specification, active Work Packet, release gate, recurring operational need, or material risk requires a durable representation. Do not activate the artifact merely because the catalog contains this contract.
...
This contract governs the structure, semantics, authority, lifecycle, traceability, and verification of Error Message Guidelines records used by Monad. It does not grant decision authority, approve an implementation, or supersede higher-order governance. Tool-specific UI fields are projections unless explicitly designated canonical.
...
- audience, prerequisites, and safety assumptions.
- executable steps with expected results.
- failure diagnosis, rollback/recovery, and escalation.
...
Verification for an instantiated Error Message Guidelines includes:
