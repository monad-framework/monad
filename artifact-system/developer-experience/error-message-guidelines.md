...
This artifact defines the core contract; it also defines repeatable operating guidance, decision points, examples, cautions, and escalation paths; and defines error classes, stable codes, safe disclosure, retry semantics, and remediation. It exists within Monad's developer experience concern, which covers the end-to-end experience of installing, learning, configuring, using, debugging, upgrading, and contributing to Monad. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Error Message Guidelines** dependable. The primary quality concerns are onboarding, feedback, errors, discoverability, migration.
...
- Error Message Guidelines has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/developer-experience/error-message-guidelines.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
