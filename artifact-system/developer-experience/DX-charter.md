...
**Status:** Draft  
**Artifact class:** Developer Experience / Charter  
...
This artifact defines mandate, boundaries, stakeholders, decision rights, and success conditions. It exists within Monad's developer experience concern, which covers the end-to-end experience of installing, learning, configuring, using, debugging, upgrading, and contributing to Monad. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Dx Charter** dependable. The primary quality concerns are onboarding, feedback, errors, discoverability, migration.
...
- Dx Charter has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/developer-experience/DX-charter.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
