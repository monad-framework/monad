...
In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Onboarding Guide** dependable. The primary quality concerns are onboarding, feedback, errors, discoverability, migration.
...
- Onboarding Guide has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/developer-experience/onboarding-guide.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
