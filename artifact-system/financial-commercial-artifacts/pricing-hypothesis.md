...
In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Pricing Hypothesis** dependable. The primary quality concerns are sustainability, cost, packaging, licensing, service obligations.
...
- Pricing Hypothesis has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/financial-commercial-artifacts/pricing-hypothesis.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
