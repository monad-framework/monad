...
In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Support Model** dependable. The primary quality concerns are sustainability, cost, packaging, licensing, service obligations.
...
- Support Model has an explicit scope and must not silently absorb neighboring responsibilities.
...
The support model model defines named concepts, their stable identities, ownership, lifecycle, and invariants.
...
`artifact-system/financial-commercial-artifacts/support-model.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
