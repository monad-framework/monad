...
**Status:** Draft  
**Artifact class:** API and Protocol / Strategy  
...
This artifact defines objectives, sequencing, trade-offs, constraints, and measures used to guide execution. It exists within Monad's api and protocol concern, which covers versioned contracts through which Monad components, tools, agents, plugins, services, and external systems exchange commands, events, state, and evidence. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **SDK Strategy** dependable. The primary quality concerns are contract stability, errors, compatibility, authentication, observability.
...
- SDK Strategy has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/api-and-protocol-artifacts/SDK-strategy.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
