...
**Status:** Draft  
**Artifact class:** API and Protocol / Engineering Artifact  
...
In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Public Internal API Classification** dependable. The primary quality concerns are contract stability, errors, compatibility, authentication, observability.
...
- Public Internal API Classification has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/api-and-protocol-artifacts/public-internal-API-classification.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
