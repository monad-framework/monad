...
This artifact defines which producers and consumers may interoperate and how incompatible change is detected and migrated. It exists within Monad's api and protocol concern, which covers versioned contracts through which Monad components, tools, agents, plugins, services, and external systems exchange commands, events, state, and evidence. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Compatibility Guarantees** dependable. The primary quality concerns are contract stability, errors, compatibility, authentication, observability.
...
`artifact-system/api-and-protocol-artifacts/compatibility-guarantees.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
