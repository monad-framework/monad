...
**Status:** Draft  
**Artifact class:** Domain Model / Model  
...
This artifact defines the core contract; it also defines entities, relationships, states, invariants, ownership, and allowed transitions; and defines states, transition authority, review triggers, retention, deprecation, and retirement. It exists within Monad's domain model concern, which covers the ubiquitous language, bounded contexts, entities, value objects, aggregates, invariants, events, ownership, and lifecycle rules that define Monad's problem domain. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Lifecycle Models** dependable. The primary quality concerns are language, identity, ownership, invariants, events.
...
- Lifecycle Models has an explicit scope and must not silently absorb neighboring responsibilities.
...
The lifecycle models model defines named concepts, their stable identities, ownership, lifecycle, and invariants.
...
`artifact-system/domain/lifecycle-models.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
