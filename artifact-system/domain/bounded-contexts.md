...
This artifact defines how task-relevant knowledge is selected, minimized, ordered, bounded, and proven current. It exists within Monad's domain model concern, which covers the ubiquitous language, bounded contexts, entities, value objects, aggregates, invariants, events, ownership, and lifecycle rules that define Monad's problem domain. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Bounded Contexts** dependable. The primary quality concerns are language, identity, ownership, invariants, events.
...
- Bounded Contexts has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/domain/bounded-contexts.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
