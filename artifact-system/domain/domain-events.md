...
This artifact defines the core contract; it also defines event identity, schema, ordering, delivery, compatibility, privacy, and consumer obligations; and defines model-agnostic AI integration boundaries, safety, data handling, provenance, evaluation, cost, and human authority. It exists within Monad's domain model concern, which covers the ubiquitous language, bounded contexts, entities, value objects, aggregates, invariants, events, ownership, and lifecycle rules that define Monad's problem domain. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Domain Events** dependable. The primary quality concerns are language, identity, ownership, invariants, events.
...
- Domain Events has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/domain/domain-events.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
