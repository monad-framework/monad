...
This artifact defines the stable responsibilities, rules, evidence, and change boundaries for this artifact. It exists within Monad's product strategy concern, which covers the product thesis, users, jobs, positioning, capabilities, boundaries, roadmap, adoption, ecosystem, and success measures for Monad. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Required Capabilities** dependable. The primary quality concerns are user outcomes, differentiation, scope, roadmap, evidence.
...
- Required Capabilities has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/product-increments/required-capabilities.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
