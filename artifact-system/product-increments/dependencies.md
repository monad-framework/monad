...
In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Dependencies** dependable. The primary quality concerns are user outcomes, differentiation, scope, roadmap, evidence.
...
- Dependencies has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/product-increments/dependencies.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
