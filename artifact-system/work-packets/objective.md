...
In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Objective** dependable. The primary quality concerns are explicit scope, stable identity, traceability, review, verification.
...
- Objective has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/work-packets/objective.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
