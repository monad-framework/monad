...
**Status:** Draft  
**Artifact class:** Historical Record / Architecture  
...
This artifact defines structural responsibilities, boundaries, flows, interfaces, deployment concerns, and trade-offs. It exists within Monad's historical record concern, which covers durable records that explain how Monad evolved, including superseded designs, releases, migrations, incidents, retrospectives, and rejected proposals. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Major Architecture Transitions** dependable. The primary quality concerns are chronology, provenance, supersession, lessons, preservation.
...
- Major Architecture Transitions has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/historical-record/major-architecture-transitions.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
