...
In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Rejected Proposals** dependable. The primary quality concerns are chronology, provenance, supersession, lessons, preservation.
...
- Rejected Proposals has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/historical-record/rejected-proposals.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
