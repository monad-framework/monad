...
This artifact defines entities, relationships, states, invariants, ownership, and allowed transitions. It exists within Monad's registry concern, which covers discovery, metadata, trust, publication, mirroring, federation, compatibility, retention, and governance for distributed Monad ecosystem artifacts. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Trust Model** dependable. The primary quality concerns are namespace, metadata, trust, discovery, compatibility.
...
`artifact-system/registry-artifacts/trust-model.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
