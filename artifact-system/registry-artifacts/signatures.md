...
In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Signatures** dependable. The primary quality concerns are namespace, metadata, trust, discovery, compatibility.
...
- Signatures has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/registry-artifacts/signatures.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
