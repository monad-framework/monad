...
**Catalog path:** `artifact-system/registry-artifacts/package-artifact-metadata.md`  
**Status:** Draft  
...
The **Package Artifact Metadata** artifact makes package artifact metadata explicit and reviewable within Monad. It exists to prevent important registry artifacts decisions from living only in chat, memory, implementation detail, or tool-specific state.

Within the artifact catalog this document defines the purpose, required information, authority, lifecycle, relationships, validation, and safe use of this engineering artifact. Its family focuses on distribution and discovery of trusted schemas, plugins, policies, templates, metadata, signing, namespaces, retention, and federation. The contract is deliberately independent of a particular implementation tool so humans, ChatGPT, Codex, CI, and future Monad automation can reason about the same engineering intent.

## Activation

Create or promote an instantiated Package Artifact Metadata when an accepted decision, approved specification, active Work Packet, release gate, recurring operational need, or material risk requires a durable representation. Do not activate the artifact merely because the catalog contains this contract.
...
Verification for an instantiated Package Artifact Metadata includes:
