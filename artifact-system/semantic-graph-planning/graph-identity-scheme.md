...
- Graph Identity Scheme has an explicit scope and must not silently absorb neighboring responsibilities.
...
The graph identity scheme model defines named concepts, their stable identities, ownership, lifecycle, and invariants.
...
`artifact-system/semantic-graph-planning/graph-identity-scheme.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
