...
**Status:** Draft  
**Artifact class:** Testing / Engineering Artifact  
...
This artifact defines the core contract; it also defines the invariants required for equivalent inputs and environment declarations to produce equivalent results; and defines the evidence and environment capture required to independently reproduce a result. It exists within Monad's testing concern, which covers unit, integration, system, acceptance, contract, conformance, property, fuzz, performance, security, compatibility, migration, and determinism verification. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Deterministic Reproducibility Testing** dependable. The primary quality concerns are risk-based coverage, repeatability, fixtures, evidence, failure paths.
...
- Deterministic Reproducibility Testing has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/testing-strategy/deterministic-reproducibility-testing.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
