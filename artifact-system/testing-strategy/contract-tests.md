...
In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Contract Tests** dependable. The primary quality concerns are risk-based coverage, repeatability, fixtures, evidence, failure paths.
...
- Contract Tests has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/testing-strategy/contract-tests.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
