...
**Catalog path:** `artifact-system/testing-strategy/contract-tests.md`  
**Status:** Draft  
...
The **Contract Tests** artifact makes contract tests explicit and reviewable within Monad. It exists to prevent important testing strategy decisions from living only in chat, memory, implementation detail, or tool-specific state.

Within the artifact catalog this document defines explicit obligations and guarantees at a boundary, including failures, evidence, and compatibility. Its family focuses on unit, integration, system, conformance, regression, determinism, property, security, performance, acceptance, and test data strategy. The contract is deliberately independent of a particular implementation tool so humans, ChatGPT, Codex, CI, and future Monad automation can reason about the same engineering intent.

## Activation

Create or promote an instantiated Contract Tests when an accepted decision, approved specification, active Work Packet, release gate, recurring operational need, or material risk requires a durable representation. Do not activate the artifact merely because the catalog contains this contract.
...
This contract governs the structure, semantics, authority, lifecycle, traceability, and verification of Contract Tests records used by Monad. It does not grant decision authority, approve an implementation, or supersede higher-order governance. Tool-specific UI fields are projections unless explicitly designated canonical.
...
Verification for an instantiated Contract Tests includes:
