...
This artifact defines which producers and consumers may interoperate and how incompatible change is detected and migrated. It exists within Monad's testing concern, which covers unit, integration, system, acceptance, contract, conformance, property, fuzz, performance, security, compatibility, migration, and determinism verification. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Compatibility Testing** dependable. The primary quality concerns are risk-based coverage, repeatability, fixtures, evidence, failure paths.
...
- Compatibility Testing has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/testing-strategy/compatibility-testing.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
