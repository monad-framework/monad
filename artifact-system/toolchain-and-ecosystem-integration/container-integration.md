...
This artifact defines model-agnostic AI integration boundaries, safety, data handling, provenance, evaluation, cost, and human authority. It exists within Monad's ecosystem concern, which covers the repository, package, SDK, plugin, schema, release, and organizational relationships that allow Monad to evolve beyond a single repository without losing coherence. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Container Integration** dependable. The primary quality concerns are repository boundaries, shared contracts, release coordination, ownership, compatibility.
...
- Container Integration has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/toolchain-and-ecosystem-integration/container-integration.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
