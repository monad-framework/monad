...
In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Python Integration** dependable. The primary quality concerns are repository boundaries, shared contracts, release coordination, ownership, compatibility.
...
- Python Integration has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/toolchain-and-ecosystem-integration/Python-integration.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
