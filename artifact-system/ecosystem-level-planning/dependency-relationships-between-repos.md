...
This artifact defines the core contract; it also defines allowed relationship types, direction, cardinality, invariants, and provenance; and defines inventory, ownership, version, risk, update, compatibility, and exit requirements. It exists within Monad's ecosystem concern, which covers the repository, package, SDK, plugin, schema, release, and organizational relationships that allow Monad to evolve beyond a single repository without losing coherence. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Dependency Relationships Between Repos** dependable. The primary quality concerns are repository boundaries, shared contracts, release coordination, ownership, compatibility.
...
- Dependency Relationships Between Repos has an explicit scope and must not silently absorb neighboring responsibilities.
...
The dependency relationships between repos model defines named concepts, their stable identities, ownership, lifecycle, and invariants.
...
`artifact-system/ecosystem-level-planning/dependency-relationships-between-repos.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
