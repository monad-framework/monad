...
**Status:** Draft  
**Artifact class:** Engineering Journal / Index  
...
This artifact defines the authoritative navigation surface for a class of records and the rules for keeping it current. It exists within Monad's engineering journal concern, which covers the engineering journal concern within Monad's engineering knowledge compilation and operating model. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Journal Index** dependable. The primary quality concerns are explicit scope, stable identity, traceability, review, verification.
...
- Journal Index has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/engineering-journal/journal-index.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
