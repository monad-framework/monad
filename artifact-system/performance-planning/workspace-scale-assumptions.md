...
**Status:** Draft  
**Artifact class:** Performance / Engineering Artifact  
...
This artifact defines repository discovery, ownership, package/component boundaries, identity, configuration, and local state. It exists within Monad's performance concern, which covers latency, throughput, memory, startup, graph scale, workspace scale, profiling, benchmarking, and performance-regression governance. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Workspace Scale Assumptions** dependable. The primary quality concerns are budgets, benchmarks, representative load, regression, capacity.
...
- Workspace Scale Assumptions has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/performance-planning/workspace-scale-assumptions.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
