...
In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Memory Budgets** dependable. The primary quality concerns are budgets, benchmarks, representative load, regression, capacity.
...
- Memory Budgets has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/performance-planning/memory-budgets.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
