...
This artifact defines the stable responsibilities, rules, evidence, and change boundaries for this artifact. It exists within Monad's legal and compliance concern, which covers legal, licensing, privacy, trademark, contribution, export, and compliance obligations that constrain distribution and operation. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Terms For Hosted Services** dependable. The primary quality concerns are obligations, evidence, ownership, licensing, review.
...
- Terms For Hosted Services has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/legal-compliance-artifacts-if-applicable/terms-for-hosted-services.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
