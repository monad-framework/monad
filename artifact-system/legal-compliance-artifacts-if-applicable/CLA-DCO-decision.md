...
In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Cla Dco Decision** dependable. The primary quality concerns are obligations, evidence, ownership, licensing, review.
...
- Cla Dco Decision has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/legal-compliance-artifacts-if-applicable/CLA-DCO-decision.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
