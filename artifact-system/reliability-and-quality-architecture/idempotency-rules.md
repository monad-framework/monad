...
**Status:** Draft  
**Artifact class:** Quality and Reliability / Rules  
...
In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Idempotency Rules** dependable. The primary quality concerns are correctness, determinism, failure handling, recovery, evidence.
...
- Idempotency Rules has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/reliability-and-quality-architecture/idempotency-rules.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
