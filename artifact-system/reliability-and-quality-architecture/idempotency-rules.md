...
**Catalog path:** `artifact-system/reliability-and-quality-architecture/idempotency-rules.md`  
**Status:** Draft  
...
The **Idempotency Rules** artifact makes idempotency rules explicit and reviewable within Monad. It exists to prevent important reliability and quality architecture decisions from living only in chat, memory, implementation detail, or tool-specific state.
...
Create or promote an instantiated Idempotency Rules when an accepted decision, approved specification, active Work Packet, release gate, recurring operational need, or material risk requires a durable representation. Do not activate the artifact merely because the catalog contains this contract.
...
Verification for an instantiated Idempotency Rules includes:
