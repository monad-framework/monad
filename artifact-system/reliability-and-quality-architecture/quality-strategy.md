...
In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Quality Strategy** dependable. The primary quality concerns are correctness, determinism, failure handling, recovery, evidence.
...
- Quality Strategy has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/reliability-and-quality-architecture/quality-strategy.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
