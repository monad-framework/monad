...
**Status:** Draft  
**Artifact class:** Quality and Reliability / Plan  
...
This artifact defines a sequenced, owned path from current state to an explicit outcome and decision gate. It exists within Monad's quality and reliability concern, which covers correctness, determinism, reproducibility, resilience, degradation, recovery, concurrency, resource, and failure-model expectations. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Corruption Recovery Plan** dependable. The primary quality concerns are correctness, determinism, failure handling, recovery, evidence.
...
- Corruption Recovery Plan has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/reliability-and-quality-architecture/corruption-recovery-plan.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
