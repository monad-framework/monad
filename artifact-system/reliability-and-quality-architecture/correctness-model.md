...
In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Correctness Model** dependable. The primary quality concerns are correctness, determinism, failure handling, recovery, evidence.
...
- Correctness Model has an explicit scope and must not silently absorb neighboring responsibilities.
...
The correctness model model defines named concepts, their stable identities, ownership, lifecycle, and invariants.
...
`artifact-system/reliability-and-quality-architecture/correctness-model.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
