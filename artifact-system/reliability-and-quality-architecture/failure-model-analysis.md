...
**Status:** Draft  
**Artifact class:** Quality and Reliability / Model  
...
This artifact defines the core contract; it also defines entities, relationships, states, invariants, ownership, and allowed transitions; and defines model-agnostic AI integration boundaries, safety, data handling, provenance, evaluation, cost, and human authority. It exists within Monad's quality and reliability concern, which covers correctness, determinism, reproducibility, resilience, degradation, recovery, concurrency, resource, and failure-model expectations. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Failure Model Analysis** dependable. The primary quality concerns are correctness, determinism, failure handling, recovery, evidence.
...
- Failure Model Analysis has an explicit scope and must not silently absorb neighboring responsibilities.
...
The failure model analysis model defines named concepts, their stable identities, ownership, lifecycle, and invariants.
...
`artifact-system/reliability-and-quality-architecture/failure-model-analysis.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
