...
**Status:** Draft  
**Artifact class:** Decision Management / Register  
...
This artifact defines a controlled living record with stable IDs, ownership, state, review triggers, and closure criteria. It exists within Monad's decision management concern, which covers the lifecycle and traceability of material project and engineering decisions from question and evidence through approval, supersession, and review. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Decision Register** dependable. The primary quality concerns are decision rights, evidence, alternatives, consequences, supersession.
...
`artifact-system/decision-management/decision-register.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
