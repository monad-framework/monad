...
**Catalog path:** `artifact-system/decision-management/decision-register.md`  
**Status:** Draft  
...
The **Decision Register** artifact makes decision register explicit and reviewable within Monad. It exists to prevent important decision management decisions from living only in chat, memory, implementation detail, or tool-specific state.

Within the artifact catalog this document defines a controlled inventory, identifiers, ownership, state, update rules, and integrity checks. Its family focuses on decision capture, authority, options, evidence, dissent, lifecycle, supersession, and discoverability. The contract is deliberately independent of a particular implementation tool so humans, ChatGPT, Codex, CI, and future Monad automation can reason about the same engineering intent.
