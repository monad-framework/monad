...
**Status:** Draft  
**Artifact class:** Decision Management / Review Contract  
...
This artifact defines entry criteria, reviewers, evidence, findings, disposition, decision authority, and closure conditions. It exists within Monad's decision management concern, which covers the lifecycle and traceability of material project and engineering decisions from question and evidence through approval, supersession, and review. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Decision Review Dates** dependable. The primary quality concerns are decision rights, evidence, alternatives, consequences, supersession.
...
- Decision Review Dates has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/decision-management/decision-review-dates.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
