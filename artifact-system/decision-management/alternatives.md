...
In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Alternatives** dependable. The primary quality concerns are decision rights, evidence, alternatives, consequences, supersession.
...
- Alternatives has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/decision-management/alternatives.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
