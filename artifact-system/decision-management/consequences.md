...
In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Consequences** dependable. The primary quality concerns are decision rights, evidence, alternatives, consequences, supersession.
...
- Consequences has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/decision-management/consequences.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
