...
This artifact defines allowed relationship types, direction, cardinality, invariants, and provenance. It exists within Monad's decision management concern, which covers the lifecycle and traceability of material project and engineering decisions from question and evidence through approval, supersession, and review. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Supersession Relationships** dependable. The primary quality concerns are decision rights, evidence, alternatives, consequences, supersession.
...
- Supersession Relationships has an explicit scope and must not silently absorb neighboring responsibilities.
...
The supersession relationships model defines named concepts, their stable identities, ownership, lifecycle, and invariants.
...
`artifact-system/decision-management/supersession-relationships.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
