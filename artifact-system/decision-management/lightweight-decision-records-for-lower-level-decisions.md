...
**Status:** Draft  
**Artifact class:** Decision Management / Engineering Artifact  
...
This artifact defines the stable responsibilities, rules, evidence, and change boundaries for this artifact. It exists within Monad's decision management concern, which covers the lifecycle and traceability of material project and engineering decisions from question and evidence through approval, supersession, and review. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Lightweight Decision Records For Lower Level Decisions** dependable. The primary quality concerns are decision rights, evidence, alternatives, consequences, supersession.
...
- Lightweight Decision Records For Lower Level Decisions has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/decision-management/lightweight-decision-records-for-lower-level-decisions.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
