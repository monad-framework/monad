...
**Catalog path:** `artifact-system/community-open-source-planning/issue-triage-policy.md`  
**Status:** Draft  
...
The **Issue Triage Policy** artifact makes issue triage policy explicit and reviewable within Monad. It exists to prevent important community open source planning decisions from living only in chat, memory, implementation detail, or tool-specific state.
...
Create or promote an instantiated Issue Triage Policy when an accepted decision, approved specification, active Work Packet, release gate, recurring operational need, or material risk requires a durable representation. Do not activate the artifact merely because the catalog contains this contract.
...
This contract governs the structure, semantics, authority, lifecycle, traceability, and verification of Issue Triage Policy records used by Monad. It does not grant decision authority, approve an implementation, or supersede higher-order governance. Tool-specific UI fields are projections unless explicitly designated canonical.
...
Verification for an instantiated Issue Triage Policy includes:
