...
In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **RFC Process** dependable. The primary quality concerns are maintainership, contribution, transparency, security reporting, sustainability.
...
- RFC Process has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/community-open-source-planning/RFC-process.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
