...
**Status:** Draft  
**Artifact class:** Community and Open Source / Guide  
**Owner:** Product Owner  
...
This artifact defines repeatable operating guidance, decision points, examples, cautions, and escalation paths. It exists within Monad's community and open source concern, which covers the participation, contribution, maintenance, governance, release, support, and ecosystem practices required for a sustainable open-source project. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Contributor Guide** dependable. The primary quality concerns are maintainership, contribution, transparency, security reporting, sustainability.
...
- Contributor Guide has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/community-open-source-planning/contributor-guide.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
