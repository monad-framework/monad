...
**Status:** Draft  
**Artifact class:** Dependency Management / Policy  
...
This artifact defines the core contract; it also defines mandatory or conditionally mandatory rules, enforcement points, exceptions, and evidence; and defines inventory, ownership, version, risk, update, compatibility, and exit requirements. It exists within Monad's dependency management concern, which covers inventory, ownership, selection, update, compatibility, licensing, security, and lifecycle controls for external and internal dependencies. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Runtime Dependency Policy** dependable. The primary quality concerns are ownership, versioning, supply chain, compatibility, exit strategy.
...
- Runtime Dependency Policy has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/dependency-management/runtime-dependency-policy.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
