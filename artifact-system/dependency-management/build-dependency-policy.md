...
In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Build Dependency Policy** dependable. The primary quality concerns are ownership, versioning, supply chain, compatibility, exit strategy.
...
- Build Dependency Policy has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/dependency-management/build-dependency-policy.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
