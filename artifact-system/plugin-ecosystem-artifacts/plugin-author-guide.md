...
**Catalog path:** `artifact-system/plugin-ecosystem-artifacts/plugin-author-guide.md`  
**Status:** Draft  
...
The **Plugin Author Guide** artifact makes plugin author guide explicit and reviewable within Monad. It exists to prevent important plugin ecosystem artifacts decisions from living only in chat, memory, implementation detail, or tool-specific state.

Within the artifact catalog this document defines executable guidance, prerequisites, steps, expected outcomes, failures, recovery, and escalation. Its family focuses on extension contracts, manifests, capability boundaries, discovery, isolation, permissions, signing, compatibility, and conformance. The contract is deliberately independent of a particular implementation tool so humans, ChatGPT, Codex, CI, and future Monad automation can reason about the same engineering intent.

## Activation

Create or promote an instantiated Plugin Author Guide when an accepted decision, approved specification, active Work Packet, release gate, recurring operational need, or material risk requires a durable representation. Do not activate the artifact merely because the catalog contains this contract.
...
This contract governs the structure, semantics, authority, lifecycle, traceability, and verification of Plugin Author Guide records used by Monad. It does not grant decision authority, approve an implementation, or supersede higher-order governance. Tool-specific UI fields are projections unless explicitly designated canonical.
...
Verification for an instantiated Plugin Author Guide includes:
