...
This artifact defines entities, relationships, states, invariants, ownership, and allowed transitions. It exists within Monad's plugin ecosystem concern, which covers extension contracts, discovery, capabilities, permissions, sandboxing, compatibility, signing, publication, and certification for third-party integrations. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Installation Model** dependable. The primary quality concerns are manifest, permissions, isolation, compatibility, trust.
...
- Installation Model has an explicit scope and must not silently absorb neighboring responsibilities.
...
The installation model model defines named concepts, their stable identities, ownership, lifecycle, and invariants.
...
`artifact-system/plugin-ecosystem-artifacts/installation-model.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
