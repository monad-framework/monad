...
**Status:** Draft  
**Artifact class:** Plugin Ecosystem / Engineering Artifact  
...
This artifact defines the core contract; it also defines callable boundaries, requests, responses, errors, versioning, authentication, and observability; and defines extension discovery, manifest, capability, permissions, lifecycle, compatibility, isolation, and trust. It exists within Monad's plugin ecosystem concern, which covers extension contracts, discovery, capabilities, permissions, sandboxing, compatibility, signing, publication, and certification for third-party integrations. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Plugin API** dependable. The primary quality concerns are manifest, permissions, isolation, compatibility, trust.
...
- Plugin API has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/plugin-ecosystem-artifacts/plugin-API.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
