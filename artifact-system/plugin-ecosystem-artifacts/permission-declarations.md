...
This artifact defines capabilities, subjects, resources, scopes, delegation, denial semantics, and audit evidence. It exists within Monad's plugin ecosystem concern, which covers extension contracts, discovery, capabilities, permissions, sandboxing, compatibility, signing, publication, and certification for third-party integrations. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Permission Declarations** dependable. The primary quality concerns are manifest, permissions, isolation, compatibility, trust.
...
- Permission Declarations has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/plugin-ecosystem-artifacts/permission-declarations.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
