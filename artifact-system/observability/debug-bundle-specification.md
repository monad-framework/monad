...
**Status:** Draft  
**Artifact class:** Observability / Specification  
...
This artifact defines the stable responsibilities, rules, evidence, and change boundaries for this artifact. It exists within Monad's observability concern, which covers structured logs, metrics, traces, diagnostics, execution records, and explainability evidence needed to understand Monad's behavior and failures. The document turns that concern into an explicit, reviewable engineering contract rather than leaving it in chat history, tribal knowledge, tool defaults, or implementation accidents.

## Scope

In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Debug Bundle Specification** dependable. The primary quality concerns are correlation, structured signals, privacy, debugging, retention.
...
- Debug Bundle Specification has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/observability/debug-bundle-specification.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
