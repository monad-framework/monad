...
In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Telemetry Governance** dependable. The primary quality concerns are correlation, structured signals, privacy, debugging, retention.
...
- Telemetry Governance has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/observability/telemetry-governance.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
