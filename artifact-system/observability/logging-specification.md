...
**Catalog path:** `artifact-system/observability/logging-specification.md`  
**Status:** Draft  
...
The **Logging Specification** artifact makes logging specification explicit and reviewable within Monad. It exists to prevent important observability decisions from living only in chat, memory, implementation detail, or tool-specific state.
...
Create or promote an instantiated Logging Specification when an accepted decision, approved specification, active Work Packet, release gate, recurring operational need, or material risk requires a durable representation. Do not activate the artifact merely because the catalog contains this contract.
...
Verification for an instantiated Logging Specification includes:
