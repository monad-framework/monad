...
In scope are the semantics, responsibilities, evidence, lifecycle, and interfaces directly needed to make **Logging Specification** dependable. The primary quality concerns are correlation, structured signals, privacy, debugging, retention.
...
- Logging Specification has an explicit scope and must not silently absorb neighboring responsibilities.
...
`artifact-system/observability/logging-specification.md` is the human-readable canonical source for this artifact once approved. Any representation under `machine/` is a deterministic derivative and must not be edited independently.
