...
Establish a stable, machine-readable semantic foundation for the Engineering Operating System. Every first-class EOS entity shares one canonical contract for identity, lifecycle, versioning, timestamps, relationships, authority, and provenance.
...
- `L2_NORMATIVE` — approved project rule or contract.
...
The existing EOS CLI validates current TSV registries with a small operational schema engine. Those schemas remain in place. The canonical schemas live under `.eos/schemas/core/` and are normative for the domain model, preventing this tranche from breaking current PI/WC/WP/CR/REL/MNT operations. Future migration may project canonical entities into richer registries without changing their IDs.

## Canonical source

`.eos/domain-model.json` is the registry of entity kinds, ID namespaces, lifecycle states, schema locations, authority semantics, provenance rules, and compatibility metadata. `.eos/schemas/core/base-entity.schema.json` defines the common contract. Every first-class entity has its own Draft 2020-12 JSON Schema.
